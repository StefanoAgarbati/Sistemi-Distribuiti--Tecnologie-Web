...
public class EditQuestionController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		EditQuestion edit = EditQuestion.create(qid,tid, ownerId);
		request.setAttribute("edit", edit);
		request.getRequestDispatcher(Pages.editQuestion).forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<PossibleResponse> responses = this.getResponses(request, names, question.getQuestionId());
		this.showQuestion(question);
		responses.forEach(res -> showResponse(res));
		this.questionService.updateQuestion(question, responses);
		String tid = request.getParameter("tid");
		if(tid!=null) 
			this.redirectTo(response, "EditTest?tid="+tid);
		else
			this.redirectTo(response, "questions");
	}
	...
	private PossibleResponse getPossibleResponse(HttpServletRequest request, String name, String questionId) {
		...
		return PossibleResponse.create(responseId, questionId, isCorrect,
				responseText, name);
	}
